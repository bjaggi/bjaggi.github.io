...
function toV3(acct) {
    ...
    if (Array.isArray(acct.keyPlayers)) {
        return {
            ...
            attachments: (acct.attachments || []).filter(function (a) { return a && a.driveFileId; })
        };
    }
    var imp = '';
...
function mergePullWithLocal(profiles, localAccounts) {
    ...
    return deduplicateByName(profiles.map(function (p) {
        ...
        if (local) {
            ...
            pulled.attachments = local.attachments || [];
        }
        return pulled;
    }));
}
...
// ═══════════════════════════════════════════════
//  12. Attachments — data model & persistence
// ═══════════════════════════════════════════════

describe('Attachments — data model & persistence', () => {

    const sampleAttachments = [
        { name: 'architecture.png', driveFileId: 'abc123', mimeType: 'image/png', webViewLink: 'https://drive.google.com/file/d/abc123/view', thumbnailLink: 'https://lh3.google.com/thumb', size: 204800, uploadedAt: '2026-04-20' },
        { name: 'proposal.pdf', driveFileId: 'def456', mimeType: 'application/pdf', webViewLink: 'https://drive.google.com/file/d/def456/view', thumbnailLink: '', size: 1048576, uploadedAt: '2026-04-21' }
    ];

    test('toV3 preserves attachments array', () => {
        const acct = {
            id: 'a1', name: 'Test', keyPlayers: [],
            attachments: sampleAttachments
        };
        const out = toV3(acct);
        assert.equal(out.attachments.length, 2);
        assert.equal(out.attachments[0].driveFileId, 'abc123');
        assert.equal(out.attachments[1].name, 'proposal.pdf');
    });

    test('toV3 filters out invalid attachments (no driveFileId)', () => {
        const acct = {
            id: 'a1', name: 'Test', keyPlayers: [],
            attachments: [
                { name: 'good.png', driveFileId: 'xyz', mimeType: 'image/png' },
                { name: 'bad.png' },
                null,
                { name: 'also-bad.png', driveFileId: '' }
            ]
        };
        const out = toV3(acct);
        assert.equal(out.attachments.length, 1);
        assert.equal(out.attachments[0].driveFileId, 'xyz');
    });

    test('toV3 defaults to empty array when no attachments', () => {
        const out = toV3({ keyPlayers: [], name: 'NoFiles' });
        assert.deepEqual(out.attachments, []);
    });

    test('attachments survive JSON roundtrip (save/load)', () => {
        const acct = toV3({
            id: 'att1', name: 'WithFiles', keyPlayers: [],
            attachments: sampleAttachments
        });
        const json = JSON.stringify([acct]);
        const loaded = JSON.parse(json).map(toV3);
        assert.equal(loaded[0].attachments.length, 2);
        assert.equal(loaded[0].attachments[0].driveFileId, 'abc123');
        assert.equal(loaded[0].attachments[0].name, 'architecture.png');
        assert.equal(loaded[0].attachments[0].mimeType, 'image/png');
        assert.equal(loaded[0].attachments[0].size, 204800);
        assert.equal(loaded[0].attachments[1].driveFileId, 'def456');
        assert.equal(loaded[0].attachments[1].uploadedAt, '2026-04-21');
    });

    test('attachments are preserved during Drive pull-merge (local-only field)', () => {
        const local = [{
            id: 'l1', name: 'Nvidia', keyPlayers: [],
            useCases: [], architectureText: '', architectureImage: '', valuePath: [],
            attachments: sampleAttachments
        }];
        const pulled = [{
            accountName: 'Nvidia',
            keyPlayers: [{ name: 'Tony', title: 'VP', other: '' }],
            topInitiatives: ['New from Drive'],
            nextSteps: [], additionalNotes: []
        }];
        const merged = mergePullWithLocal(pulled, local);
        assert.equal(merged[0].attachments.length, 2);
        assert.equal(merged[0].attachments[0].driveFileId, 'abc123');
        assert.equal(merged[0].attachments[1].driveFileId, 'def456');
    });

    test('new account from Drive gets empty attachments', () => {
        const pulled = [{
            accountName: 'NewCorp',
            keyPlayers: [], topInitiatives: [], nextSteps: [], additionalNotes: []
        }];
        const merged = mergePullWithLocal(pulled, []);
        assert.deepEqual(merged[0].attachments, []);
    });

    test('attachments are NOT serialized to Google Doc text (local-only)', () => {
        const account = {
            name: 'FileCorp', keyPlayers: [],
            topInitiatives: [], nextSteps: [], additionalNotes: [],
            attachments: sampleAttachments
        };
        const text = DP.serializeAccountToProfileText(account);
        assert.ok(!text.includes('abc123'), 'driveFileId should not be in doc text');
        assert.ok(!text.includes('architecture.png'), 'attachment name should not be in doc text');
    });

    test('attachment metadata fields are all preserved', () => {
        const att = {
            name: 'diagram.svg', driveFileId: 'svg999',
            mimeType: 'image/svg+xml',
            webViewLink: 'https://drive.google.com/file/d/svg999/view',
            thumbnailLink: 'https://lh3.google.com/thumb/svg999',
            size: 51200, uploadedAt: '2026-05-01'
        };
        const acct = toV3({ id: 'x', name: 'MetaTest', keyPlayers: [], attachments: [att] });
        const loaded = JSON.parse(JSON.stringify(acct));
        const a = loaded.attachments[0];
        assert.equal(a.name, 'diagram.svg');
        assert.equal(a.driveFileId, 'svg999');
        assert.equal(a.mimeType, 'image/svg+xml');
        assert.equal(a.webViewLink, 'https://drive.google.com/file/d/svg999/view');
        assert.equal(a.thumbnailLink, 'https://lh3.google.com/thumb/svg999');
        assert.equal(a.size, 51200);
        assert.equal(a.uploadedAt, '2026-05-01');
    });

    test('multiple accounts each keep their own attachments through merge', () => {
        const local = [
            { id: 'l1', name: 'Nvidia', keyPlayers: [], useCases: [], architectureText: '', architectureImage: '', valuePath: [], attachments: [sampleAttachments[0]] },
            { id: 'l2', name: 'Cisco', keyPlayers: [], useCases: [], architectureText: '', architectureImage: '', valuePath: [], attachments: [sampleAttachments[1]] }
        ];
        const pulled = [
            { accountName: 'Nvidia', keyPlayers: [], topInitiatives: [], nextSteps: [], additionalNotes: [] },
            { accountName: 'Cisco', keyPlayers: [], topInitiatives: [], nextSteps: [], additionalNotes: [] }
        ];
        const merged = mergePullWithLocal(pulled, local);
        assert.equal(merged[0].attachments.length, 1);
        assert.equal(merged[0].attachments[0].name, 'architecture.png');
        assert.equal(merged[1].attachments.length, 1);
        assert.equal(merged[1].attachments[0].name, 'proposal.pdf');
    });
});
