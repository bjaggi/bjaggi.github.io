...
(function (global) {
    ...
    var ATTACHMENTS_FOLDER_PREFIX = '[Account Brain] ';

    function findOrCreateFolder(token, accountName, cb) {
        var folderName = ATTACHMENTS_FOLDER_PREFIX + accountName;
        var q = "name='" + folderName.replace(/'/g, "\\'") + "' and mimeType='application/vnd.google-apps.folder' and trashed=false";
        driveSearch(token, q, function (err, files) {
            if (err) return cb(err);
            if (files && files.length) return cb(null, files[0].id);
            fetch('https://www.googleapis.com/drive/v3/files', {
                method: 'POST',
                headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
                body: JSON.stringify({ name: folderName, mimeType: 'application/vnd.google-apps.folder' })
            })
                .then(function (r) { return r.json().then(function (d) { if (!r.ok) throw apiError(r, d, 'Create folder failed'); return d; }); })
                .then(function (d) { cb(null, d.id); })
                .catch(cb);
        });
    }

    function uploadFileToDrive(token, folderId, file, cb) {
        var metadata = { name: file.name, parents: [folderId] };
        var formData = new FormData();
        formData.append('metadata', new Blob([JSON.stringify(metadata)], { type: 'application/json' }));
        formData.append('file', file);
        fetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,mimeType,webViewLink,thumbnailLink,size', {
            method: 'POST',
            headers: { Authorization: 'Bearer ' + token },
            body: formData
        })
            .then(function (r) { return r.json().then(function (d) { if (!r.ok) throw apiError(r, d, 'Upload failed'); return d; }); })
            .then(function (d) { cb(null, d); })
            .catch(cb);
    }

    function uploadAttachment(accountName, file, cb) {
        getAccessToken(function (err, token) {
            if (err) return cb(err);
            findOrCreateFolder(token, accountName, function (e2, folderId) {
                if (e2) return cb(e2);
                uploadFileToDrive(token, folderId, file, function (e3, driveFile) {
                    if (e3) return cb(e3);
                    cb(null, {
                        name: driveFile.name,
    ...
    function deleteAttachment(driveFileId, cb) {
        getAccessToken(function (err, token) {
            if (err) return cb(err);
            fetch('https://www.googleapis.com/drive/v3/files/' + encodeURIComponent(driveFileId), {
                method: 'DELETE',
                headers: { Authorization: 'Bearer ' + token }
            })
                .then(function (r) {
    ...
    function listFolderFiles(token, folderId, cb) {
        var q = "'" + folderId + "' in parents and trashed=false";
        fetch('https://www.googleapis.com/drive/v3/files?q=' + encodeURIComponent(q) +
            '&fields=files(id,name,mimeType,webViewLink,thumbnailLink,size,createdTime)&pageSize=100', {
            headers: { Authorization: 'Bearer ' + token }
        })
            .then(function (r) { return r.json().then(function (d) { if (!r.ok) throw apiError(r, d, 'List files failed'); return d.files || []; }); })
            .then(function (files) { cb(null, files); })
            .catch(cb);
    }

    function listAccountAttachments(accountName, cb) {
        getAccessToken(function (err, token) {
            if (err) return cb(err);
            var folderName = ATTACHMENTS_FOLDER_PREFIX + accountName;
            var q = "name='" + folderName.replace(/'/g, "\\'") + "' and mimeType='application/vnd.google-apps.folder' and trashed=false";
            driveSearch(token, q, function (e2, folders) {
                if (e2) return cb(e2);
                if (!folders || !folders.length) return cb(null, []);
                listFolderFiles(token, folders[0].id, cb);
            });
        });
    }
    ...
    global.AccountBrainDrive = {
        ...
        listAccountAttachments: listAccountAttachments
    };
